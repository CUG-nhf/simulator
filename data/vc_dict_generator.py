...
def main(args):
	...
	if cluster == 'Philly':
		df = pd.read_csv(Path(f'/data/nihaifeng/HeliosData/data/Philly/philly_vc.csv'))
	else:
	...
	if cluster == 'Uranus' and date == 'Sept':
		vcs = df.columns.values[1:-1]
		nodes_num = df[df['date'] == datetime.datetime(
			2020, 9, 10)].values[0][1:-1]
	...
	if cluster == 'Philly':
		# Philly did not release VC configurations, the size of each VC is set corresponding to its workloads
...
if __name__ == '__main__':
	...
	parser.add_argument('-d', '--date', default='Sept',
						type=str, help='Month')
