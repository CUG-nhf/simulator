...
class DotProdPlacement:
	...
	def nodeSelect(self, job, job_gpu_num, node_list):
		...
		'''assign partially idle nodes -- DotProd'''
		avail_nodes = [node for node in nodes if node.free_gpus >= partial_node_nmu]
		targe_node, minDotProd = None, sys.float_info.max
		for node in avail_nodes:
			if node.free_gpus * partial_node_nmu < minDotProd:
				targe_node, minDotProd = node, node.free_gpus * partial_node_nmu
		if targe_node is not None:
			alloc_nodes.append((targe_node, partial_node_nmu))
			return True, alloc_nodes
		return False, alloc_nodes
